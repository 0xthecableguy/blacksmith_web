...
export const requestTextToSpeech = async (payload: TextToSpeechRequest): Promise<TextToSpeechResponse> => {
	...
	const response = await fetch(`${API_BASE}/blacksmith_web_tts_request`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
		},
...
export const fetchChatHistory = async (userId: string, app_name: string): Promise<ChatMessage[]> => {
	...
	const response = await fetch(`${API_BASE}/blacksmith_web_chat_fetch?user_id=${userId}&app_name=${app_name}`, {
		method: 'GET',
